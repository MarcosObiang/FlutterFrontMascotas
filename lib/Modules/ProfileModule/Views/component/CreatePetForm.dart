...
Future<void> _submitForm() async {
  ...
  try {
    ...
    print('🚀 Iniciando creación de mascota desde formulario...');
    
    // Llamar al método del viewModel y esperar el resultado
    bool success = await widget.viewModel.addNewPet(
    ...
    print('✅ Resultado de creación: $success');
    
    if (success && mounted) {
      print('🎉 Mascota creada exitosamente, mostrando mensaje y cerrando modal');
      ...
      // Llamar al callback de éxito que cerrará el modal
      widget.onSuccess();
    } else if (!success) {
      print('⚠️ La creación no fue exitosa según el ViewModel');
      if (mounted) {
        setState(() {
          _errorMessage = 'La mascota no se pudo crear correctamente. Intenta de nuevo.';
          _isLoading = false;
        });
      }
    }
  } catch (e) {
    print('❌ Error capturado en formulario: $e');
    
    if (mounted) {
      // Verificar si el error es solo de procesamiento local
      String errorMessage = e.toString();
      
      // Si es un error de procesamiento local pero la mascota se creó
      if (errorMessage.contains('Error al procesar') || 
          errorMessage.contains('podría haberse creado')) {
        
        // Mostrar mensaje de advertencia pero cerrar el modal
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Mascota creada, pero con algunos problemas de sincronización. Revisa tu lista de mascotas.'),
        ...
        // Error real
        setState(() {
          _errorMessage = 'Error al crear mascota. Verifica tu conexión e intenta de nuevo.';
          _isLoading = false;
        });
      }
    }
